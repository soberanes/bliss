...
use Cshelperzfcuser\Form\Recovery;
use Cshelperzfcuser\Form\CompleteRef;
use Cshelperzfcuser\Form\CompleteDist;
    ...
    public function recoveryAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                $userDao = $this->getServiceLocator()->get('zfcuser_user_mapper');
                $user = $userDao->findByEmail($formData['email']);
                if ($user) {
                    $sender = $this->getServiceLocator()->get('mailer_sender_service');
                    $send = $sender->sendByUserIdRecovery($user->getId());
                }
            }
        }
        return new ViewModel(array('form' => $form, 'send' => $send));
    }
